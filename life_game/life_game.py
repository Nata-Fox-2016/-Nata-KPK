...
frame_sleep_time = 1   # zadergka v mili sek
...
screen_width = cell_width * cells_horizontal_number    # shirina igrovogo ekrana
screen_height = cell_height * cells_vertical_number    # visota igrovogo ekrana
...
class Field:
    def __init__(self, field_file, canvas):
        """zagrujaet pole s kletkami iz faila"""
        self._canvas = canvas
        with open(field_file) as file:
            ...
            for yi in range(cells_vertical_number):
                ...
                for xi in range(cells_horizontal_number):
                    # luboi simvol, krome probela -- znachit sootvet. kletka jiva
    ...
    def calculate(self):
        """  """
        # raschitivaem matrisu sostoyanii kletok na sleduyusem shage
        ...
        for yi in range(1, cells_vertical_number-1):
            for xi in range(1, cells_horizontal_number-1):
                # gjlschitivaem kol-vo jivix sosedei
                ...
                if (cell_is_alive and number_of_neighbours == 2) or number_of_neighbours == 3:
                    ...
                    new_matrix[yi][xi] = 0
        # kopiruem raschitannuyu matrisu v self.matrix
        for yi in range(1, cells_vertical_number-1):
            for xi in range(1, cells_horizontal_number-1):
...
def time_event():
    global scores
    # perevichislit sostoyanie polya s kletkami
    field.calculate()
...
if __name__ == "__main__":
    ...
    time_event()  # nachinayu siklicheski zapuskat taimer
    root.mainloop()